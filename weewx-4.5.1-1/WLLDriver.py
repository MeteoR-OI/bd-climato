            ...
            while start_timestamp <= end_timestamp:
                ...
                for sensor in self.dict_device_id:
                    ...
                    for sensor_type_id in self.dict_sensor_type[sensor]:
                        logdbg('sensor_type_id: %s' % sensor_type_id)
                        for q in data_wl['sensors']:
                            if q['sensor_type'] == sensor_type_id:
                                logdbg('q[sensor_type_id]: %s' % q['sensor_type'])
                                for pk_sensor in q['data']:
                                    logdbg('pk_sensor: %s' % pk_sensor)
                                    if pk_sensor['tx_id'] == self.dict_device_id[sensor] \
                                            and pk_sensor['ts'] == start_timestamp:
                                        logdbg('check_key: %s' % check_key)
                                        logdbg('self.list_iss: %s' % self.list_iss) 
                                        # modified by QUETELARD
                                        if check_key in self.list_iss:
                                            ...
                                            logdbg('wl_packet[outTemp]: %s' % wl_packet['outTemp'])
                                        ...
                                        # added by QUETELARD
                                        if check_key in self.list_iss or check_key in self.list_anemometer:
