...
def viewControlSvc(request):
    ...
    try:
        ...
        if (my_cde & SvcRequestType.List) == SvcRequestType.List:
            if list_services(j_resp) is False:
                return
        if j_request['trace_flag'] is not None:
            svc_instance = getSvcInstance(j_request['svc'])
            svc_instance.SetTraceFlag(j_request['trace_flag'])
            j_resp['status'] = 'ok'
            if j_request['trace_flag'] is True:
...
def ControlSvc(j_resp: dict, svc_instance, cde: SvcRequestType):
    try:
        ...
        j_resp['status'] = 'err'
        j_resp['errMsg'] = 'invalid parameter'
        return False
    except Exception as inst:
        j_resp['status'] = 'err'
