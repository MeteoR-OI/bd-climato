    ...
    def loadDataInObs(
        ...
        if b_exclu is False:
            ...
            if my_value_avg is None and (isFlagged(my_measure['special'], MeasureProcessingBitMask.MeasureIsSum)):
                if data_src.__contains__(src_key + '_s'):
                    my_value_avg = my_measure['dataType'](data_src[src_key + '_s'])
        ...
        tmp_s_avg_old = tmp_duration_old = 0
        if obs_j.__contains__(target_key):  # in obs only _avg are stored
            ...
            tmp_s_avg_old = tmp_value_old_avg * tmp_duration
            if (isFlagged(my_measure['special'], MeasureProcessingBitMask.MeasureIsSum)):
                tmp_s_avg = tmp_value_old_avg
            delta_values[target_key + '_s_old'] = tmp_s_avg_old
        ...
        tmp_s_avg = my_value_avg * tmp_duration
        if (isFlagged(my_measure['special'], MeasureProcessingBitMask.MeasureIsSum)):
            tmp_s_avg = my_value_avg

        # pass delta values to our aggregations
        delta_values[target_key + '_s'] = tmp_s_avg
