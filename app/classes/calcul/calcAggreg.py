...
class CalcAggreg():
    ...
    def ComputeAggregFromSvc(self, data: json, is_killed):
        """entry point called from worker"""
        self.stopRequested = False

        # load params
        if data is None or data.get("param") is None or data["param"] == {}:
            t.logError("ComputeAggregFromSvc", "wrong data")
        params = data["param"]
    ...
    @transaction.atomic
    def __processTodo(self, a_todo, trace_flag: bool = False):
        ...
        trace_flag = RefManager.GetInstance().GetRef("calcAggreg_trace_flag") if not None else False
                        ...
                        with self.tracer.start_span("level " + anAgg) as span_lvl:
                            ...
                            for an_instrument in all_instr.get_all_instruments():
                                ...
                                for my_measure in an_instrument["object"].get_all_measures():
                                    ...
                                    self.agg_compute.loadDVDataInAggregation(my_measure, agg_decas, delta_values, dv_next, trace_flag)
    ...
    def add_duration_to_agg(self, aggregations, mini_level, tmp_duration, span_load_data):
        """
            add_duration_to_agg

            add measure duration from a given level (mini_level) to upper levels

            an_agg will decide if the duration should be added in duration_sum or not
        """
        b_first_level_passed = False
        for an_agg in aggregations:
            if b_first_level_passed is False:
                b_first_level_passed = (mini_level == an_agg.agg_niveau)
